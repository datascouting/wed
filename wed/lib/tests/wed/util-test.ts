  ...
  describe("decodeAttrName", () => {
    ...
    it("with dashes in the name", () => {
      expect(
        ...
        .to.deep.equal({
        name: "btw:blah-one--two---three----four",
        qualifier: undefined
      });
    });
...
_xmlns_http\\:\\/\\/mangalamresearch\\.org\\/ns\\/btw-storage._real");
  ...
  describe("convertPatternObj", () => {
    ...
    it("converts an AnyName", () => {
      const anyName = new AnyName();
      assert.equal(util.convertPatternObj(anyName.toObject(), nr), "*:*");
    });
    ...
    it("converts a NameChoice", () => {
      const first = new Name("uri", "name");
      const second = new Name("uri2", "name2");
