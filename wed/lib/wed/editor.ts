...
import {ErrorItemHandler, ValidationController,} from "./validation-controller";
...
cannot be cut.`, {type: "danger"});
  ...
  private mouseoverHandler(ev: JQueryMouseEventObject): void {
    ...
    if (label !== null) {
      ...
      const tooltipOptions = new TooltipOptionsImplBuilder()
        .setTitle((): string => {
          const mode = this.modeTree.getMode(label);
          return mode.shortDescriptionFor(origName) as string;
        })
        .setContainer("body")
        .setDelay(new DelayImplBuilder().setShow(1000).build())
        .setPlacement("auto")
        .setTrigger("hover")
        .build();
      this.makeGUITreeTooltip($(label), tooltipOptions);
  ...
  private refreshSaveStatus(): void {
    if (this.saver !== undefined) {
      ...
      if (saveStatus === undefined) {
        ...
        this.$saveStatus.tooltip(tooltipOptions);
      }
