...
const CDN_BASE_URL = (typeof getCDNBaseURL === "undefined")
  ? "" :
  // eslint-disable-next-line no-undef
  `${getCDNBaseURL()}`;
const CDN_LIBRARY_URL = `${CDN_BASE_URL}/lib/`;

const APP_BASE_URL = (typeof getAppBaseURL === "undefined")
  ? "" :
  // eslint-disable-next-line no-undef
  `${getAppBaseURL()}`;
const APP_LIBRARY_URL = `${APP_BASE_URL}/lib/`;

require.config({
  baseUrl: CDN_LIBRARY_URL,
  paths: {
    ...
    bootbox: "external/bootbox.all.min",
  ...
  map: {
    "*": {
      ...
      "wed/modes/generic/metadata-schema.json": `json!${APP_LIBRARY_URL}wed/modes/generic/metadata-schema.json`,
